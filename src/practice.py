...
def input_amount(message): 
    try: 
        ...
        amt = stdscr.getstr(0, 40, 16) #row, column, length of input string
    except: 
        raise 
    finally: 
        curses.endwin() # to restore the terminal to its original operating mode.
    return amt.decode("utf-8")  # Decode the bytes to a string

while True:
    # Prompt user to enter plaintext which must be 16 characters long 
    plaintext = input_amount("Enter plaintext (16 characters):           ")
...
while True:
    ...
    secret_key = input_amount("Enter secret key (16 characters): ")
